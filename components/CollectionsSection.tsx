...
//           .select("*")
//           .eq("is_active", true)
//           .order("id", { ascending: true }); // Keeps them in order

//         if (error) throw error;

//         // Map the database names to match what your UI expects
//         const mappedCollections: Collection[] = data.map((item: any) => ({
//           id: item.id,
//           title: item.title,
//           subtitle: item.subtitle || "",
//           image: item.image_url || "/watch-2.png", // Fallback image if empty
//           bg: item.bg_color || "bg-gradient-to-br from-gray-900 to-black" // Fallback background
//         }));
    ...
    const fetchCollections = async () => {
      try {
        ...
        const mappedCollections: Collection[] = data.map((item: any) => ({
          ...
          title: item.title,
          subtitle: item.subtitle || "",
          image: item.image_url || "/watch-2.png", 
          bg: item.bg_color || "bg-gradient-to-br from-gray-900 to-black",
          type: item.type || "watch" // Map the new type column
        }));
